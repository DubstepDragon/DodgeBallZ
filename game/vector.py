...
def dot(v,w):
    """
    Calculates the dot product of two vectors and returns a scalar
    :param v: Vector 1
    :param w: Vector 2
    :return: total Dot Product
    """
...
def cross(v, w):
    """
    Calculates the Cross Product of two vectors and returns the new vector
    :param v: Vector 1
    :param w: Vector 2
    ...
    if isinstance(v, Vector) and isinstance(w, Vector):
        if len(v) == len(w):
            if isinstance(v, Vector2):
                m = Vector3(v.x, v.y, 0)  #convert vector 2 to vector 3
                k = Vector3(w.x, w.y, 0)
            ...
            raise TypeError("Vectors must be of same length")
    else:
        raise TypeError("Arguments must be vectors")
